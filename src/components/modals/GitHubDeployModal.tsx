...
export const GitHubDeployModal: React.FC<GitHubDeployModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'vercel' | 'quickstart' | 'deploy' | 'commands' | 'export' | 'video'>('vercel');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-750 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-800 bg-slate-950/30 px-6 pt-2 overflow-x-auto scrollbar-none">
          <button
            onClick={() => setActiveTab('vercel')}
            className={`pb-2.5 px-4 text-xs font-semibold border-b-2 transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
              activeTab === 'vercel'
          ...
          >
            ...
            <span>Update in Vercel</span>
            <span className="px-1.5 py-0.2 rounded-full bg-emerald-950 text-emerald-300 text-[9px] font-bold border border-emerald-800">
              CONFIGURED
            </span>
          </button>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          {activeTab === 'vercel' && (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-emerald-500/40 rounded-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-black border border-slate-750 flex items-center justify-center text-white shadow-md">
                      <Triangle className="w-5 h-5 fill-white" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-bold text-white">Vercel Deployment &amp; Update Guide</h3>
                        <span className="px-2 py-0.5 rounded-full bg-emerald-950 text-emerald-400 border border-emerald-700/80 text-[10px] font-semibold flex items-center gap-1">
                          <Check className="w-3 h-3" /> Vercel Ready
                        </span>
                      </div>
                      <p className="text-xs text-slate-300 mt-0.5">
                        Your project is pre-configured with <code className="text-emerald-300 font-mono">vercel.json</code> and serverless API handlers in <code className="text-emerald-300 font-mono">/api/</code>.
                      </p>
                    </div>
                  </div>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3 py-1.5 bg-white text-black hover:bg-slate-100 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm shrink-0"
                  >
              ...
              <div className="space-y-3">
                <div className="text-xs font-bold text-slate-200 uppercase tracking-wider flex items-center gap-2">
                  ...
                  <span>Choose How You Want to Update in Vercel:</span>
                </div>

                {/* Option 1: Automatic Push from GitHub */}
                <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-emerald-500/20 border border-emerald-400/50 text-emerald-300 text-xs font-bold flex items-center justify-center">
                        1
                      </span>
                      <h4 className="text-xs font-bold text-white">
                        Recommended: Push to GitHub (Auto-Deploys in ~30s)
                      </h4>
                    </div>
                  ...
                  <p className="text-xs text-slate-300">
                    If your project is connected to GitHub on Vercel, push any commit to your repository. Vercel detects the push and deploys the new version automatically.
                  </p>
                  <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 font-mono text-[11px] text-slate-200 flex items-center justify-between">
                    <div className="space-y-0.5">
                      <div><span className="text-slate-500"># Commit and push updates</span></div>
                      <div>git add .</div>
                      <div>git commit -m &quot;Update circuit designer &amp; clipboard tools&quot;</div>
                      <div>git push origin main</div>
                    </div>
                    <button
                      onClick={() => copyToClipboard('git add .\ngit commit -m "Update circuit designer & clipboard tools"\ngit push origin main', 'gitupdate')}
                      className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded text-xs font-medium border border-slate-700 flex items-center gap-1 cursor-pointer shrink-0 transition-colors"
                    >
                      {copiedKey === 'gitupdate' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                      <span>{copiedKey === 'gitupdate' ? 'Copied' : 'Copy'}</span>
                    </button>
                  </div>
                </div>

                {/* Option 2: Instant Update via Vercel CLI */}
                <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-sky-500/20 border border-sky-400/50 text-sky-300 text-xs font-bold flex items-center justify-center">
                        2
                      </span>
                      <h4 className="text-xs font-bold text-white">
                        Vercel CLI: Deploy Instantly from Terminal
                      </h4>
                    </div>
                  ...
                  <p className="text-xs text-slate-300">
                    Deploy directly to production without waiting for a git push using the official Vercel CLI tool:
                  </p>
                  <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 font-mono text-[11px] text-slate-200 flex items-center justify-between">
                    <div>
                      <span className="text-emerald-400">npx vercel</span> <span className="text-white font-bold">--prod</span>
                    </div>
                    <button
                      onClick={() => copyToClipboard('npx vercel --prod', 'vercelcli')}
                      className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded text-xs font-medium border border-slate-700 flex items-center gap-1 cursor-pointer shrink-0 transition-colors"
                    >
                      {copiedKey === 'vercelcli' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                      <span>{copiedKey === 'vercelcli' ? 'Copied' : 'Copy Command'}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-purple-500/20 border border-purple-400/50 text-purple-300 text-xs font-bold flex items-center justify-center">
                        3
                      </span>
                      <h4 className="text-xs font-bold text-white">
                        Vercel Dashboard: 1-Click Redeploy
                      </h4>
                    </div>
                    <a
                      href="https://vercel.com/dashboard"
                  ...
                  <ol className="text-xs text-slate-300 space-y-1 list-decimal list-inside">
                    <li>Go to <strong className="text-white">vercel.com/dashboard</strong> and click on your project.</li>
                    <li>Open the <strong className="text-white">Deployments</strong> tab.</li>
                    <li>Click the <strong className="text-white">... (three dots)</strong> next to the latest deployment and select <strong className="text-purple-300">Redeploy</strong>.</li>
                  </ol>
                </div>
              </div>

              {/* Vercel Configuration Details & Verification */}
              <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-xl space-y-3">
                <h4 className="text-xs font-bold text-slate-200 flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" />
                  <span>Pre-Configured Vercel Architecture Status</span>
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
                  <div className="p-2.5 bg-slate-900/80 rounded-lg border border-slate-800 space-y-1">
                    ...
                    <p className="text-[11px] text-slate-400">
                      Framework: <code className="text-emerald-300 font-mono">vite</code>, Output: <code className="text-emerald-300 font-mono">dist</code>, Build: <code className="text-emerald-300 font-mono">npm run vercel-build</code>
                    </p>
                  </div>
                  ...
                  <div className="p-2.5 bg-slate-900/80 rounded-lg border border-slate-800 space-y-1">
                    <div className="font-semibold text-slate-200 flex items-center gap-1.5">
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                      <span>Serverless Functions (/api)</span>
                    </div>
                    <p className="text-[11px] text-slate-400">
                      <code className="text-sky-300 font-mono">/api/circuit/generate</code>, <code className="text-sky-300 font-mono">/api/google/search</code>, <code className="text-sky-300 font-mono">/api/circuit/explain</code>, <code className="text-sky-300 font-mono">/api/health</code>
                    </p>
                  </div>
                </div>
                ...
                <div className="p-3 bg-amber-950/20 border border-amber-800/40 rounded-lg text-xs space-y-1">
                  <div className="font-semibold text-amber-300 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                    <span>Optional: Gemini API Key in Vercel</span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    If you wish to use Gemini AI for generating circuits, go to your Vercel Project &rarr; <strong className="text-white">Settings</strong> &rarr; <strong className="text-white">Environment Variables</strong>, and add <code className="text-amber-300 font-mono">GEMINI_API_KEY</code>. Even without an API key, the built-in EDA rule engine handles synthesis automatically!
                  </p>
                </div>
              </div>
            </div>
