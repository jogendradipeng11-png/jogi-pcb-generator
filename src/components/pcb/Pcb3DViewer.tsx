...
export const Pcb3DViewer: React.FC<Pcb3DViewerProps> = ({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    setRotX((prev) => Math.max(0, Math.min(85, prev - dy * 0.4)));
    setDragStart({ x: e.clientX, y: e.clientY });
  };
  ...
  // Map components to 3D board grid (Board dimensions: 540px x 380px)
  const boardWidth = 540;
  const boardHeight = 380;
  const cols = Math.max(3, Math.ceil(Math.sqrt(components.length * 1.5)));

  const pcbItems = components.map((c, idx) => {
    const col = idx % cols;
    const row = Math.floor(idx / cols);
    const x = 70 + col * ((boardWidth - 140) / Math.max(1, cols - 1));
    const y = 60 + row * ((boardHeight - 120) / Math.max(1, Math.ceil(components.length / cols) - 1));
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-slate-900/90 backdrop-blur-xs border-b border-slate-800 text-xs z-20">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            {showEnclosure ? 'Hide Product Enclosure' : 'Product Enclosure View'}
          </button>
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Copper Traces Layer (Under solder mask) */}
              <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-85">
                {pcbItems.map((c, i) => {
                  if (i === pcbItems.length - 1) return null;
                  const next = pcbItems[i + 1];
                  const midX = (c.posX + next.posX) / 2;
                  return (
                    <g key={`trace-${i}`}>
                      <path
                        d={`M ${c.posX} ${c.posY} L ${midX} ${c.posY} L ${midX} ${next.posY} L ${next.posX} ${next.posY}`}
                        fill="none"
                        stroke={theme.copperTrace}
                        strokeWidth="3.2"
                        strokeLinecap="round"
                      ...
                      />
                      {/* Solder Via */}
                      <circle cx={midX} cy={(c.posY + next.posY) / 2} r="2.8" fill="#eab308" stroke="#050914" strokeWidth="1" />
                    </g>
                  );
                })}
              </svg>
                ...
                return (
                  <div
                    ...
                    className={`absolute cursor-pointer group transition-all ${
                      isSelected ? 'ring-2 ring-sky-400 ring-offset-2 ring-offset-slate-900 rounded' : ''
                    }`}
                  >
